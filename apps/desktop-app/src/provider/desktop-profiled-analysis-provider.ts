...
export class DesktopProfiledAnalysisProvider implements AnalysisProvider {
  ...
  async analyze(
    ...
    const result = parseWritingModelResult(
      await adapter.analyze(requestFrom(snapshot), {
        profile,
        secret,
        signal,
      }),
  ...
  #toProposal(
    ...
    const generationGroupId = asGenerationGroupId(
      `desktop-profile-generation-${idSuffix}`,
    );
    const outputs = [];
    ...
    if (result.nativeIntent !== null) {
      outputs.push(
        Object.freeze({
          id: asTrackId(`desktop-profile-native-${idSuffix}`),
    ...
    for (const [index, normalized] of result.normalized.entries()) {
      outputs.push(
        Object.freeze({
          id: asTrackId(
            `desktop-profile-normalized-${idSuffix}-${index + 1}`,
          ),
