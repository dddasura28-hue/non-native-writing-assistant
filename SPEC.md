...
Windows External Host Adapter v1 is the first real adapter over this boundary. Windows Global Trigger & Floating Assistant v1 invokes that adapter through one explicit system shortcut and shows its result in a compact read-only app window. It does not provide realtime global assistance or external replacement.
...
The Windows desktop app registers exactly one manual shortcut, `Ctrl+Alt+Space`, through the Rust side of the official Tauri 2 global-shortcut plugin. On each press, Rust performs a fresh UIA capture of the still-focused external control and fixes the snapshot before emitting any frontend event or making the hidden `global-assistant` window visible. The previous three-second development button is not a product path.

The startup-created assistant window is hidden initially, always on top, omitted from the taskbar, and non-focusable on platforms that honor that Tauri window property. It is reused across invocations and placed at the bottom-right of the primary monitor work area; v1 does not infer caret screen geometry. A new usable capture replaces the previous presentation, while an unsupported, protected, own-process, or failed capture clears prior content and keeps the window hidden. Closing the assistant hides it rather than destroying its WebView.

The floating presentation is read-only and contains only safe lifecycle status, captured Source, the first Native Intent, and the first Normalized variant in existing presentation order. It has no Accept, confirmation, settings, recent-assistance, or provider controls. The existing `GlobalDesktopAssistantController`, `DesktopProfiledAnalysisProvider`, active BYOK profile, capability policy, cancellation, and stale-result rules remain authoritative. If configuration is missing, capture may complete but analysis does not call the provider and the panel shows a compact configuration-required state. Rapid invocations carry monotonically increasing identity so an older provider result or delayed presentation cannot repaint the current host.

This shortcut is an explicit manual trigger. `canObserveComposition` and `canReplaceText` remain false for Windows UIA captures, so cross-application realtime and external Accept remain disabled. Protected text is never serialized, analyzed, displayed, logged, or persisted. There is no clipboard, SendKeys, SendInput, simulated typing, UIA event subscription, background polling, system tray workflow, customizable hotkey, or caret-anchored external overlay.
