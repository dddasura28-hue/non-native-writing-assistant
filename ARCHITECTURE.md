...
Inline Native Intent editing or confirmation, recent-unit cards, explicit-selection positioning, textarea ghost text, rich-editor migration, inline diffs, completion decorations, keyboard Accept shortcuts, system-global overlays, clipboard access, UI Automation, TSF, and InputMethodKit remain deferred.
...
The external presentation contains only availability, read-only and capability-derived action state, captured source, safe lifecycle status, and the existing assistance presentation. It includes no window title, process name, app identity, native handle, accessibility object, geometry, or secret, so none of that data can enter a dependency stamp or provider request. V1 adds no polling, event hook, global shortcut, clipboard permission, system overlay, caret geometry, Windows UI Automation, macOS Accessibility, TSF, or InputMethodKit.
