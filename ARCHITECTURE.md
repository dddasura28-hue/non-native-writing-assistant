...
`GlobalDesktopAssistantController` implements manual invocation without duplicating writing semantics. Every action requests a fresh capture, disposes the prior invocation, and creates a manual-only `DesktopEngineController` over the existing provider and configuration source. The normal selector, segmenter, coordinators, dependency stamps, presentation, and replacement construction remain authoritative. Invocation generations and current prepared-target object identity prevent a late old-host result or structurally identical old action from crossing into a newer capture. Configuration changes stale current output without starting an external background request.
...
The external presentation contains only availability, read-only and capability-derived action state, captured source, safe lifecycle status, and the existing assistance presentation. It includes no window title, process name, app identity, native handle, accessibility object, geometry, or secret, so none of that data can enter a dependency stamp or provider request. The boundary itself adds no polling, event hook, global shortcut, clipboard permission, system overlay, or caret geometry.

## Windows External Host Adapter v1

The first operating-system adapter lives under `apps/desktop-app/src-tauri/src/windows`. It uses the official `windows` crate behind a Windows target dependency; no Windows type crosses into React or a reusable package. A single fixed-purpose Tauri command, `capture_active_windows_text_surface`, returns either a minimal capture payload or a small safe unavailable reason. `apps/desktop-app/src/native/windows-active-text-surface.ts` validates that payload and converts it to the existing `ActiveTextSurfaceCapture`. The opaque native capture token is discarded there and never reaches the global controller, presentation, dependency stamp, or provider request.

Each command runs on Tauri's blocking executor, initializes COM as multithreaded for that invocation, creates an `IUIAutomation` client, obtains the current focused element, captures it, and balances successful COM initialization with `CoUninitialize`. There is no cached focused element, polling thread, UIA focus subscription, text-change listener, keyboard hook, or mouse hook. COM and provider failures map to safe host states; HRESULT values and source text are not placed in user-facing errors or logs.
...
UIA does not provide reliable IME composition state for this adapter, so `canObserveComposition` is always false. The pure global capability policy consequently permits explicit manual analysis and disables automatic realtime analysis. The React development harness uses one three-second timer after its button is clicked, allowing the user to refocus the external field before the native command performs a fresh capture. This timing mechanism neither monitors focus nor changes the writing architecture. The resulting read-only Source, Native Intent, Normalized tracks, capabilities, and safe status appear in a separate development section while using the same `DesktopProfiledAnalysisProvider`, configuration source, and `GlobalDesktopAssistantController` as the owned host.

The Tauri capability permission list remains empty. This adapter adds no clipboard, shell, process-launch, filesystem, global-shortcut, or generic accessibility RPC permission. Global hotkeys, a production floating assistant, system tray behavior, simulated input, clipboard replacement, UIA event monitoring, background polling, realtime cross-application analysis, TSF, InputMethodKit, macOS Accessibility, and global caret geometry remain deferred.
