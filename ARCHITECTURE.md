...
### Capture, analysis, presentation, and guarded editing
...
That completed boundary phase originally deferred the host runtime and application action. Desktop App Shell v1 now implements the host runtime, Desktop Engine Integration v1 connects its editor to existing analysis orchestration, and Accept / Replace v1 supplies the explicit guarded action. Global hotkeys, clipboard integration, production global overlays, Windows TSF, macOS Accessibility/InputMethodKit, browser extensions, mobile keyboards, accounts/cloud sync, updater/installer, and diff/merge conflict resolution remain deferred.
...
Accept reuses the host-neutral `TextReplacement` and `TextEditPort`; no desktop-specific edit command or mutable editor service is introduced. Application helpers map a cursor-local `WritingUnit.range`, which is relative to `ContextSelection.activeText`, into the captured `TextContext.text` by adding `ContextSelection.sourceRange.start`. The helper proves that the unit still slices to `unit.text` before creating the replacement. A non-empty explicit selection maps directly to its whole `ContextSelection.sourceRange` and exact `activeText`, so a multi-sentence selection is never split for application.

The desktop presentation marks each active Normalized variant with explicit eligibility and an opaque prepared target. That target binds the current unit or explicit-selection segment, source revision, unit source fingerprint where applicable, Normalized track identity and revision, full `DependencyStamp`, absolute range, and exact expected source text. The controller recreates and compares the replacement against current context immediately before invoking the port. It also requires the result and track to remain current under the existing source, confirmed-intent, context, policy, style, language, and processor fingerprints. React renders the prepared action and returns it on click; it does not inspect dependency stamps, calculate ranges, construct edits, or mutate the textarea.

The selected track's payload becomes `replacementText` exactly. Multiple current Normalized tracks therefore produce independent Accept actions, and choosing one cannot fall back to another. Recent Assistance receives no prepared targets and remains read-only. A stale source, target, dependency stamp, track, controller lifetime, or missing current captured port disables or rejects the action without fuzzy lookup or relocation.

The captured desktop port remains the final correctness boundary even after controller validation. It checks the original textarea session token and generation, composition state, full captured text, UTF-16 half-open range, and `expectedText` at mutation time. `setRangeText` replaces only that range and collapses the selection at the end of the inserted text. The one-shot port is consumed, React commits the resulting textarea value, and the host publishes a fresh context with a newly captured port. A failure performs no mutation and the controller presents one compact stale-source status with its remaining Accept actions disabled until the next observation.

The fresh observation after success creates a new source revision and clears the consumed result, old confirmed Native Intent, and the old target's transient React draft. That host-originated observation alone carries a one-shot flag suppressing its automatic analysis opportunity, which avoids a punctuation-triggered Accept loop. A concurrent or later genuine user observation carries no such flag, and explicit manual analysis remains available. This flag belongs to observation origin rather than an in-flight controller boolean, so an asynchronous host race cannot suppress a real user edit.

V1 does not add Accept All, Recent Assistance application, rejection state or analytics, persistent undo history, inline decorations, diff highlighting, a keyboard shortcut, global-application replacement, clipboard integration, UI Automation, TSF, or InputMethodKit. The textarea uses the platform range-editing API, but browser/WebView undo-stack behavior remains host-dependent and no custom history layer is added.

## Desktop Inline Assistance v1

Inline Assistance is a second desktop presentation surface over the existing `DesktopAssistancePresentation`; it is not a second analysis or state pipeline. Each presented assistance item identifies whether it belongs to a cursor-local unit or an explicit selection. React renders at most one inline surface only for the active cursor-local item, while the side Assistance panel remains the canonical detailed view. The inline surface reads the same status, ordered Normalized tracks, eligibility flag, and prepared Accept target. It never stores a private copy of wording, so source, context, profile, confirmed-intent, session, and dependency invalidation remove or update both surfaces through the controller's existing currentness rules.

The first Normalized track in existing presentation order is the compact inline variant. All variants remain visible in the side panel. The controller prepares a separate compact inline status from the same authoritative lifecycle: analyzing renders `Analyzing…`, ordinary provider failure renders a fixed safe unavailable message, and configuration or credential requirements remain detailed only in the side panel. Stale wording is removed immediately. Explicit selections, blank separators, empty input, missing active targets, active composition, and Recent Assistance have no inline surface. Native Intent editing, confirmation, reset, provider settings, and profile controls remain side-panel interactions; rerendering inline output cannot replace an in-progress draft for the same Native Intent target.

Caret measurement belongs to `apps/desktop-app/src/host/textarea-text-anchor.ts`. A transient hidden mirror copies the textarea properties that determine glyph placement and wrapping, including font, line height, spacing, alignment, direction, padding, borders, inner width, `pre-wrap`, and `overflow-wrap`. It inserts a zero-width measurement marker at the existing UTF-16 caret offset without changing or normalizing the input value. The mirror is aria-hidden, naturally unfocusable, invisible, nonselectable, and unable to receive pointer events; it is removed immediately after measurement and never becomes source state.

The host converts the measured marker rect into Writing-pane-local coordinates by subtracting textarea scroll offsets and the pane origin. A pure geometry step determines viewport visibility; offscreen anchors suppress the overlay. Text, caret, scroll, window resize, textarea resize, pane resize, and relevant font loading trigger discrete anchor measurement. Analysis content does not remeasure caret geometry. A separate placement step reads the real rendered card dimensions, observes card-size changes, clamps horizontally to the pane, prefers the gap below the active line, and moves above when the lower space is insufficient. No animation-frame polling loop runs, and the absolutely positioned card does not participate in textarea layout or wrapping.

Both inline and side-panel buttons call one React Accept handler with the existing prepared variant target. React still constructs no `TextReplacement` and performs no source mutation. The controller and captured `TextEditPort` retain every Accept / Replace guard and lifecycle behavior. Pointerdown on the inline card neither changes Source nor advances the host generation; the semantic button keeps native Enter and Space behavior. React restores textarea focus after the operation. Success keeps the caret established by `setRangeText`; stale failure keeps newer Source untouched and exposes the controller-prepared compact status. Successful replacement itself is the feedback, so no success toast or notification state is introduced.

Inline Native Intent editing or confirmation, recent-unit cards, explicit-selection positioning, textarea ghost text, rich-editor migration, inline diffs, completion decorations, keyboard Accept shortcuts, system-global overlays, clipboard access, TSF, and InputMethodKit remain deferred. Windows manual UI Automation capture is a separate external-host adapter and does not participate in inline textarea presentation.

## Global Desktop Assistant Boundary v1

Future system-wide assistance is another desktop host over the existing application contracts. `ActiveTextSurfacePort` is desktop-local and asynchronously returns either no active host or an `ActiveTextSurfaceCapture` containing `TextContext`, `HostCapabilities`, and a guarded `TextEditPort` when replacement is actually supported. Core and application packages remain unchanged and know nothing about operating-system windows, processes, accessibility elements, Tauri, screen coordinates, or host session identifiers.

The capture's `TextContext.text` is the complete coordinate space available for that invocation, even when it is only a partial window or one explicit selection. Cursor, selection, composition, context selection, writing-unit, and replacement ranges remain UTF-16 half-open offsets inside that string. A host that cannot observe selection reports `selection: null`; it cannot synthesize a range. A selected-text-only adapter that truly knows the manual selection may expose that exact text as a full-range explicit selection, allowing `LocalBlockContextSelector` to preserve existing whole-selection semantics without manufacturing before/after context.

A pure desktop capability policy derives three decisions. Manual analysis requires nonblank captured text with no known active composition. Guarded Accept additionally requires `canReplaceText` and a non-null one-shot edit port. Automatic realtime eligibility additionally requires reliable composition observation. Unknown composition therefore disables realtime; an explicit manual invocation may still analyze the exact text the user intentionally captured. Known active composition blocks both automatic and manual analysis. The owned textarea is adapted to the same capture shape and retains all four capabilities as true.

`GlobalDesktopAssistantController` implements manual invocation without duplicating writing semantics. Every action consumes one fresh `ActiveTextSurfacePort` capture, disposes the prior invocation, and creates a manual-only `DesktopEngineController` over the existing provider and configuration source. The normal selector, segmenter, coordinators, dependency stamps, presentation, and replacement construction remain authoritative. Invocation generations and current prepared-target object identity prevent a late old-host result or structurally identical old action from crossing into a newer capture. Configuration changes stale current output without starting an external background request.

The external adapter alone owns opaque native session identity. Its port must atomically revalidate the same application/control, captured text window, session generation, composition/write safety, and exact expected slice before committing. The controller never receives that identity. The deterministic fake adapter models text, selection, composition, capabilities, session invalidation, write rejection, and one-shot consumption without native calls. This proves read-only analysis, exact guarded replacement, same-text/new-session rejection, UTF-16 behavior, and late-result isolation before a platform adapter exists.

The external presentation contains only availability, read-only and capability-derived action state, captured source, safe lifecycle status, and the existing assistance presentation. It includes no window title, process name, app identity, native handle, accessibility object, geometry, or secret, so none of that data can enter a dependency stamp or provider request. The boundary itself adds no polling, UIA event hook, clipboard permission, write capability, or caret geometry.

## Windows External Host Adapter v1

The first operating-system adapter lives under `apps/desktop-app/src-tauri/src/windows`. It uses the official `windows` crate behind a Windows target dependency; no Windows type crosses into React or a reusable package. The Rust global-shortcut callback calls this adapter synchronously on a fresh worker and receives either a minimal capture payload or a small safe unavailable reason. A fixed `windows-global-capture` event carries that already-fixed snapshot to the two pre-created desktop webviews. `apps/desktop-app/src/native/windows-active-text-surface.ts` validates and consumes the snapshot once, converting it to the existing `ActiveTextSurfaceCapture`. The opaque native capture token is discarded there and never reaches the global controller, presentation, dependency stamp, or provider request.

Each shortcut capture runs to completion before the floating window is shown. A fresh worker initializes COM as multithreaded for that invocation, creates an `IUIAutomation` client, obtains the current focused element, captures it, and balances successful COM initialization with `CoUninitialize`. Only after capture does Rust notify the hidden assistant and main controller, position the window, and show it without requesting focus. There is no cached focused element, polling thread, UIA focus subscription, text-change listener, general keyboard hook, or mouse hook. COM and provider failures map to safe host states; HRESULT values and source text are not placed in user-facing errors or logs.

Before reading text, the native adapter checks process identity, `IsPassword`, enabled state, and keyboard-focusability. Elements in this process, protected controls, disabled controls, non-focusable controls, disappeared providers, and unsupported patterns are rejected. Raw process IDs, runtime IDs, window handles, titles, executable paths, COM pointers, and UIA trees remain native and are never serialized. A monotonic opaque token gives every native capture a distinct session identity, including equal text captured during a later invocation; no text history is retained.

Pattern use is deliberately narrow. TextPattern2 is queried first and its active `GetCaretRange` result is preferred. TextPattern supplies `DocumentRange`, `GetSelection`, and range-relative text. A single non-empty selection is captured exactly with a full-range selection and no invented surrounding context. A degenerate selection or reliable TextPattern2 caret anchors a bounded range extended by at most 8,192 UIA character units in each direction. UTF-16 offsets are derived by reading exact range prefixes, not by counting Rust Unicode scalar values. For multiple disjoint selections, a reliable TextPattern2 caret permits a caret-centered capture with `canObserveSelection: false`; without one, capture is unavailable. Disconnected ranges are never concatenated.

ValuePattern support is detected but does not by itself make a control capturable or writable. A ValuePattern-only control cannot supply truthful cursor/selection coordinates. TextPattern ranges can read and select text but do not prove a safe mutation path. The Windows adapter therefore always reports `canReplaceText: false`, supplies no `TextEditPort`, and treats external Normalized output as advisory. Runtime ValuePattern writing is deferred until a control-specific path can prove complete authoritative value capture, stable native identity, exact pre-write equality, one-shot use, deterministic replacement, caret placement, and post-write verification without partial failure.

UIA does not provide reliable IME composition state for this adapter, so `canObserveComposition` is always false. The pure global capability policy consequently permits explicit manual analysis and disables automatic realtime analysis. Rust registers one `Ctrl+Alt+Space` shortcut through `tauri-plugin-global-shortcut`; JavaScript receives no shortcut registration API. The main webview stages the fixed snapshot behind `ActiveTextSurfacePort`, then uses the same `DesktopProfiledAnalysisProvider`, configuration source, and `GlobalDesktopAssistantController` as the owned host. A monotonically increasing native invocation ID plus the controller's own generation ensure a late host A result cannot repaint host B. Missing provider configuration terminates after capture with a safe state and no provider call.

The startup-created `global-assistant` webview is hidden, always on top, omitted from the taskbar, and configured as non-focusable. It is reused rather than recreated. Native code positions it at the bottom-right of the primary monitor work area and never requests focus. The compact React surface renders safe status, Source, the first Native Intent, and the first Normalized variant only. It never renders Accept, Confirm, provider settings, or Recent Assistance. A new invocation clears the old visible state; an unusable or protected capture hides the window. Closing the assistant is converted to hide, while closing the main window exits the app and lets the plugin runtime unregister its shortcut.

The main `desktop-shell` capability grants only core event listen, unlisten, and targeted emit permissions. A separate `global-assistant` capability grants its read-only webview only event listen and unlisten. Global shortcut registration remains Rust-only, so no global-shortcut command permission is exposed to JavaScript. The adapter adds no clipboard, shell, process-launch, filesystem, generic accessibility RPC, or window-control permission. External Accept, system tray behavior, simulated input, clipboard replacement, UIA event monitoring, background polling, realtime cross-application analysis, customizable shortcuts, TSF, InputMethodKit, macOS Accessibility, and global caret geometry remain deferred.
...
These native BYOK operations require no plugin capability permissions because they are application-owned commands registered directly by the Rust builder. The only current capability permissions are the narrow core event permissions used by the global-assistant bridge, so no filesystem, shell, process, clipboard, or broad HTTP plugin capability is exposed. A profile/model switch persists first, changes the processor fingerprint, cancels or stales obsolete analysis, removes incompatible presentation immediately, and refreshes the current target under the existing immediate/debounce policy.
